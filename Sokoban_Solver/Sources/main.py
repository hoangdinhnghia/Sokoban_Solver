...
# Xác định thư mục gốc của dự án dựa trên vị trí file này: .../Sokoban Solver/Sources/main.py
# ROOT_DIR = .../Sokoban Solver
ROOT_DIR = Path(__file__).resolve().parent.parent
path_board = ROOT_DIR / 'Testcases'
path_checkpoint = ROOT_DIR / 'Checkpoints'

def get_boards():
	# Đảm bảo đường dẫn Testcases là tuyệt đối và tồn tại
	if not path_board.exists():
		raise FileNotFoundError(f"Không tìm thấy thư mục Testcases: {path_board}")
	os.chdir(str(path_board))
	list_boards = []
	for file in os.listdir():
		if file.endswith(".txt"):
			file_path = str(path_board / file)
...
def get_check_points():
	if not path_checkpoint.exists():
		raise FileNotFoundError(f"Không tìm thấy thư mục Checkpoints: {path_checkpoint}")
	os.chdir(str(path_checkpoint))
	list_check_point = []
	for file in os.listdir():
		if file.endswith(".txt"):
			file_path = str(path_checkpoint / file)
